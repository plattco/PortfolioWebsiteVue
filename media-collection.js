        ...
        <main class="container">
            ...
            <div class="media-controls">
                <div class="filter-bar">
                    ...
                    <input type="text" v-model="filters.searchTerm" placeholder="Search titles, publishers..." class="search-input">
                    ...
                    <button class="btn-add" @click="openSearchModal">+ Add Item</button>
                </div>
            </div>
            ...
            <div v-else class="media-grid">
                <div v-for="item in mediaItems" :key="item.id" :class="['media-card', { favorite: item.isFavorite }]">
                    <span :class="['media-type-badge', 'type-' + item.mediaType.toLowerCase()]">{{ item.mediaType }}</span>
                    ...
                    <div class="media-info">
                        ...
                        <div class="media-meta">
                            <div class="media-rating" v-if="item.rating">★ {{ item.rating }}/5</div>
                            <div class="media-price" v-if="item.price">{{ '$' + item.price.toFixed(2) }}</div>
                            <div :class="['media-status', 'status-' + item.status.toLowerCase()]">{{ item.status }}</div>
                        </div>
                    </div>
            ...
            <div :class="['modal', { active: showAddModal || showEditModal }]" @click.self="closeModal">
                <div class="modal-content">
                    ...
                    <form @submit.prevent="saveItem">
                        ...
                        <div class="form-group">
                            <label>
                                <input type="checkbox" v-model="currentItem.isFavorite"> Mark as Favorite
                            </label>
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-content modal-small">
                    ...
                    <form @submit.prevent="savePriceUpdate">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" v-model.number="newPrice" min="0" step="0.01" required placeholder="Enter new price">
                        </div>
                        <div class="form-actions">
            ...
            <div :class="['modal', { active: showSearchModal }]" @click.self="closeSearchModal">
                <div class="modal-content">
                    ...
                    <form @submit.prevent="searchMedia" class="search-form">
                    ...
                    <div class="search-results">
                        <div v-for="result in searchResults" :key="result.id" class="search-result-item" @click="selectSearchResult(result)">
                            <img :src="result.cover ? (result.cover.url.startsWith('//') ? 'https:' + result.cover.url : result.cover.url) : './assets/img/avatar.svg'" alt="Cover Art">
                            <div class="result-info">
                                <div class="result-title">{{ result.name }}</div>
                                <div class="result-platform" v-if="result.platforms && result.platforms.length > 0">{{ result.platforms[0].name }}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
`,
    ...
    methods: {
        ...
        async savePriceUpdate() {
            try {
                await axios.patch(`${this.apiUrl}/MediaItems/${this.priceUpdateItem.id}/price`, {price: this.newPrice});
